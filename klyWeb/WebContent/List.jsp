...
<body>
	...
	category 값을 BoardCategoryAction 으로 넘김  -->
		...
		<div id="rightbody"
						...
						<div>
							<div>
								...
								<button type="button" class="btn btn-success"
									onclick="location='boardLike.kly?board_num=${board.BOARD_NUM}&member_id=${loginInfo.MEMBER_ID}'">추천</button>${board.BOARD_LIKECOUNT}
								<button type="button" class="btn btn-danger"
									onclick="location='boardReport.kly?board_num=${board.BOARD_NUM}&member_id=${loginInfo.MEMBER_ID}'">신고</button>
								<button type="button" class="btn btn-dark"
									onclick="location='boardDelete.kly?board_num=${board.BOARD_NUM}&member_id=${loginInfo.MEMBER_ID}'">삭제</button>
							</div>
							<div>
			...
			<c:forEach var="board" items="${boardlist}" varStatus="status">
				<div class="card" style="width: 260px; height: 200px;">
					<div class="card-body text-center">
						<div class="videoplay">
							...
							<div>
								<span class="videoCategory">분류 :${board.BOARD_CATEGORY}</span> <span
									class="videoLike">추천:${board.BOARD_LIKECOUNT}</span> <span
									class="videoUptime">${board.BOARD_DATE}</span> <span
									class="videoReadCount">조회:${board.BOARD_READCOUNT}</span>
							</div>
						</div>
					</div>
				</div>
